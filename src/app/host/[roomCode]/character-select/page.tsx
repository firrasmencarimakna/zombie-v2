  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center relative overflow-hidden">
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-12 h-12 border-4 border-red-500 border-t-transparent rounded-full" />
        <motion.p className="absolute bottom-1/4 text-red-400 font-mono text-sm" animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity }}>{t("loading")}</motion.p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen relative overflow-hidden select-none bg-black">
      <div className="absolute top-4 left-4 z-20 hidden md:block">
        <Link href={"/"}>
          <h1 className="text-3xl md:text-5xl font-bold font-mono tracking-wider text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.7)]" style={{ textShadow: "0 0 10px rgba(239, 68, 68, 0.7)" }}>
            {t("title")}
          </h1>
        </Link>
      </div>
      ...
      <div className="relative z-10 mx-auto p-4 max-w-4xl">
        <header className="flex flex-col gap-1 mb-6 text-center mt-10">
